#!/usr/bin/env bash
# ============================================================================
#  run_im2_full_debug_all_obstacles.sh
# ----------------------------------------------------------------------------
...
#  markers.  RViz can display this all-obstacle topic alongside the normal
#  local perception topic.
#
#  This does not change the planner, fake detector service, predictor, or
#  evaluation metrics.  It only adds a MarkerArray publisher for visualization.
#
#  Usage:
#    bash $(rospack find trajectory_planner)/scripts/run_im2_full_debug_all_obstacles.sh [SEEDS] [DURATION_SEC] [GOAL_RADIUS]
#
#  The old run_im2_full_debug_all_obstacles_rviz.sh path is kept as a shim.
#
#  Useful environment overrides:
#    ALL_DYNAMIC_BBOX_TOPIC=/onboard_detector/all_dynamic_bboxes
...
ALL_DYNAMIC_BBOX_TOPIC="${ALL_DYNAMIC_BBOX_TOPIC:-/onboard_detector/all_dynamic_bboxes}"
...
LOG_ROOT="${HOME}/IM2MPPI/results/all_obstacles_overlay_${STAMP}"
mkdir -p "${LOG_ROOT}"
...
start_visualizer() {
    ...
    echo "[all-obstacles-overlay] starting all-obstacle bbox publisher -> ${ALL_DYNAMIC_BBOX_TOPIC}"
    python3 "${VIS_NODE}" \
...
echo
echo "================================================================"
echo "  IM2 full debug + all dynamic obstacle overlay"
